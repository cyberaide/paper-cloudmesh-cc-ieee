...
of the systems and identify features that are important for us and
present examples using citations to the features. The
following features are important.

%\begin{description}

\subsubsection{ Batch parallelism} Long-running jobs on HPC systems are coordinated through
Batch services  \cite{www-lsf}\cite{www-slurm} \cite{www-cloudmesh-manual}\cite{www-airflow} \cite{las-1996-thesis,las-1999-loosely}.
...
is often used in distributed resource frameworks \cite{www-cloudmesh-manual}\cite{www-airflow}.
...
implement them using the batch system  \cite{las-02-infogram}\cite{www-pegasus} \cite{www-parsl}\cite{arxiv-radical-pilot}.
...
workflows \cite{www-cloudmesh-manual}\cite{www-airflow}\cite{www-gcloud}\cite{www-azure-rest}\cite{www-aws-workflow}\cite{www-azure-batch}.
...
of the workflows  \cite{www-snakemake}\cite{las-2004-gridant}\cite{www-pegasus}\cite{las-2007-swift}\cite{www-aws-swf}.

\subsubsection{ GUI} Some frameworks provide extensive GUIs \cite{www-kepler}\cite{www-aws-stepfunctions}\cite{www-azure-batch} \cite{www-azure-enterprise-workflow}.

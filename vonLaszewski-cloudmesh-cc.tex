...
{\bf Simplicity.} The design must be simple and the code base must be
...
{\bf Workflow specification.} The specification of the workflow must
...
{\bf Workflow Monitoring.} The workflow framework must be able to
monitor the progress of individual jobs as well as the overall
progress of the workflow.

{\bf Workflow Interfaces.} To specify and interface with the
...
{\bf Language Independence.} As we want to make the framework
...
{\bf Support of various Operating Systems.} The framework must be
runnable on various client operating systems including Linux, macOS,
Windows.

{\bf State Management.} The state management must be recoverable and
...
{\bf Batch parallelism.} Long-running jobs on HPC systems are coordinated through
Batch services.

{\bf Task parallelism.} Tasks are a logical unit of work that is
executed by a resource, service, or component. Task parallelism
is often used in distributed resource frameworks.

{\bf Resource Reservation.} In some cases, access to batch queues on
...
{\bf Computational Grids.} Grids provide service-level access to
...
{\bf Cloud.} Presently, computing resources are also available in
...
{\bf REST.} The predominant specification for cloud services uses the
REST framework relying on a stateless model. This contrasts with the
WSRF model that uses stateful services.

{\bf Specification.} Some work has also focused on the specification
of the workflows.

{\bf GUI.} Some frameworks provide extensive GUIs.

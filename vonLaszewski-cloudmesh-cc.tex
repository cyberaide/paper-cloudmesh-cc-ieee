...
\author{\IEEEauthorblockN{1\textsuperscript{st} Gregor von Laszewski\textsuperscript{\href{https://orcid.org/0000-0001-9558-179X}{\includegraphics[height=9pt]{images/orcid_icon.png}}}}
\IEEEauthorblockA{\textit{Biocomplexity Institute}\\
\textit{University of Virginia}\\
Charlottesville, VA 22911, USA \\
laszewski@gmail.com}
\and
\IEEEauthorblockN{2\textsuperscript{nd} J.P. Fleischer\textsuperscript{\href{https://orcid.org/0000-0002-1102-1910}{\includegraphics[height=9pt]{images/orcid_icon.png}}}}
\IEEEauthorblockA{\textit{Biocomplexity Institute}\\
\textit{University of Virginia}\\
Charlottesville, VA 22911, USA \\
jacquespfleischer@gmail.com}
\and
\IEEEauthorblockN{3\textsuperscript{rd} Geoffrey C. Fox\textsuperscript{\href{https://orcid.org/0000-0003-1017-1391}{\includegraphics[height=9pt]{images/orcid_icon.png}}}}
\IEEEauthorblockA{\textit{Biocomplexity Institute}\\
\textit{University of Virginia}\\
...
\begin{abstract}
 ...
 for generation of reproducible experiments while creating permutations over
 selected experiment parameters. The resulting framework was
...
\begin{IEEEkeywords}
high performance computing, batch queue, service
\end{IEEEkeywords}
...
(c) creating a reproducible experiment is often out of scope as the focus for beginning students that barely have the time to run and understand the application during a semester activity.

What we strive to achieve is to make the situation simpler for the students while working at the same time towards the implementation of the FAIR principle \cite{www-fair} for the targeted MLCommons Science applications addressing the previous mentioned issues. 
...
at \cite{las-22-arxiv-workflow-cc}. One of the systems that has been developed is MLCube \cite{www-mlcube} developed within MLCommons. Similar to our framework, it uses YAML specification for organizing, running, and sharing machine learning experiments. It attempts to standardize the 
...
Next, we list the various categories of related workflow systems while augmenting. By no means is this list complete, but it represents a selected number of efforts. 
...
\section{Specifying Task-based Workflows}\label{specifying-workflows}
...
and end node. This is a typical use pattern for DL-related analysis tasks. Dependencies can easily be specified in human-readable format while using the node name. The nodenames contain easy-to-manage information such as the name of the node, a label that is used to print the node's progress. The label may contain variables that can be custom designed and added to the workflow. Default variables that are frequently used include {\em name}, {\em progress}, and {\em now}.  Nodes can also have a type indication if it represents
a Python, sh,
...
Presently, we use Graphviz \cite{www-graphviz} for rendering the workflow. Thus we also integrated the ability to define attributes provided by Graphviz such as shapes and styles. The full palette of Graphviz features in regards to styles and styles is supported.

\subsection{Reporting Progress}\label{reporting-progress}

When running jobs inside a Cloudmesh Workflow, the scripts called can use a standard format provided by 
...
It is important to note that we have also a {\em command line mode} that interfaces directly with the backends while not requiring a service. All features are available as PythonAPI. In addition, we have used this API to implement a {\em service}, that allows us to stand up a REST service as well as a GUI that can be accessed through a Web browser.
...
To support direct execution on the command line we also provide not only a command line interface but leverage the cloudmesh command shell enabling us to interact with the system from the cloudmesh shell on-demand. 
...
Figure \ref{fig:table-view} shows the table view of a possible cloudmask workflow, while \label{fig:graph-view} shows the graph view. During execution, the information in the views will be automatically updated.
...
Cloudmesh-sbatch is a hyperparameter and configuration management toolkit designed to address the generation of batch jobs with a consistent and configurable interface based on hyperparameter values across multiple development toolchains. One of its functions is to create batch jobs based on parameterized job specifications and configuration files.  As it is integrated with cloudmesh, it can leverage a variety of queuing systems and submission commands through abstractions. Currently, we provide interfaces to Slurm, LSF, and ssh as these are the frameworks used on HPC and local resources that we have access to. However, it would be possible to integrate others due to the extensible design of our framework.

The architecture of the cloudmesh-sbatch framework is depicted in Figure \ref{fig:cc-2} and also demonstrates that it integrates with our previous cloudmesh-cc component.
...
It is important to note that for each experiment a custom batch file and configuration file is created that only includes the values for that particular experiment. 
To allow easy integration of parameters in YAML and shell scripts, we have provided a specific extension to YAML that allows to fill in the value of a variable that is defined within a YAML file. The variables can easily be referred to with a dot notation in the templates. 

One can choose experiment parameters that best suit the application and benchmark.

While practically working with the system, we observed that students not using cloudmesh-sbatch often spend a significant amount of a semester just setting up a benchmark and replicating only a subset of the features that cloudmesh-sbatch provides. However, when a student uses it, we observed that the applications for which a template and configuration file has been designed the on-ramp-time was reduced to less than a day.
